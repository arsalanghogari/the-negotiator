...
- Once you have a number, negotiate: ${[
      ...
      marketRate
        ? `You also researched the market rate for this exact job before calling: ${marketRate}. These figures come from other companies' published prices, so use them aggressively and attribute them ("from another company I found out a move like this runs about $X"): anchor early with the low end, measure every total against the range, and keep pushing toward the low end until the seller genuinely holds firm — do not settle for the first small concession. Cite only these EXACT researched figures. Two things you may never do with them: claim one is a binding offer, or claim you were personally given a quote by that company.`
        : '',
